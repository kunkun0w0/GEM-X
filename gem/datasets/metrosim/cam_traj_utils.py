...
def create_camera(w_root, cfg):
    ...
    roll = np.clip(randn() * roll_std, -halfpi, halfpi)
...
class CameraAugmentorV11:
    ...
    r_xyz_w_std = [np.pi / 6, np.pi / 3, np.pi / 6]
    t_xyz_w_std = [1.0, 0.25, 1.0]
    ...
    def __call__(self, w_j3d, length=120, camera_type=None):
        ...
        if camera_type == "random":
        ...
        elif camera_type == "track":
            R_w2c = create_rotation_move(R0_w2c, length, self.r_xyz_w_std_half)
            cam_mat = torch.inverse(transform_mat(R0_w2c, t0_w2c)).repeat(length, 1, 1)
            t_w2c = self.create_translation_track(cam_mat, w_root, 0.5)
            R_w2c, t_w2c = self.add_stepnoise(R_w2c, t_w2c)

        elif camera_type == "trackrotate":
            cam_mat = torch.inverse(transform_mat(R0_w2c, t0_w2c)).repeat(length, 1, 1)
            t_w2c = self.create_translation_track(cam_mat, w_root, 0.5)
            ...
            R_w2c = R0_w2c.repeat(length, 1, 1)
            t_w2c = t0_w2c.repeat(length, 1)
        ...
        if c_j3d_min < self.tz_post_min:
            ...
            c_j3d[..., 2] += push_away
        c_root = c_j3d[:, 0]
        half_fov = torch.div(c_root[:, :2], c_root[:, 2:]).abs()
        if half_fov.max() > self.half_fov_tol:
            ...
            z_trg = c_root[max_idx1, max_idx2].abs() / self.half_fov_tol
